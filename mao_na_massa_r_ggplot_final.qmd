...
g_vendas_por_regiao <- ggplot(vendas_por_regiao, 
                              aes(x = reorder(regiao, vendas_por_regiao), 
                                  y = vendas_por_regiao)) + 
                        geom_col(fill = red_2) +
                        scale_y_continuous(expand = c(0,0), limits = c(0, 4e6)) +
                        coord_flip()
...
g_vendas_por_regiao <- g_vendas_por_regiao +
  tema +
  labs(title = "Vendas da Zoop por Região (2023)")
...
```{r}
g_vendas_por_regiao <- g_vendas_por_regiao +
  geom_label(aes(label = number(vendas_por_regiao,
                                prefix = "R$ ",
                                scale_cut = cut_si(""))), hjust=-0.05)
g_vendas_por_regiao
```
...
# do Mão na Massa da Aula 03
g_vendas_por_tri_estados_sul <- ggplot(vendas_por_tri_estados_sul,
                                       aes(trimestre, total_tri_estados_sul, 
                                           fill = uf)) +
                                  geom_col(position = "dodge") +
...
g_vendas_por_tri_estados_sul <- g_vendas_por_tri_estados_sul +
  tema +
  labs(title = "Vendas trimestrais da Zoop na Região Sul (2023)",
       x = "", y = "Total de vendas", fill = "Estado")
...
# do Mão na Massa da Aula 03
g_vendas_por_mes_regiao <- ggplot(vendas_por_mes_regiao,
                                  aes(x = mes, y = vendas_por_mes_regiao,
...
g_vendas_por_mes_regiao <- g_vendas_por_mes_regiao +
  tema +
  labs(title = "Vendas mensais da Zoop por região (2023)",
       x = "Mês", y = "Total de vendas", fill = "Região")
...
::: {.callout-tip appearance="simple"}
Para te ajudar na criação das anotações, este [link](https://www.cedricscherer.com/2019/08/05/a-ggplot2-tutorial-for-beautiful-plotting-in-r/#text) traz ótimos exemplos de como construir visuais atrativos com ggplot2.
:::

```{r}
g_vendas_por_mes_regiao <- g_vendas_por_mes_regiao +
  annotate("text", x = 8, y = 7.2e5, vjust = 1, size = 3.5,
            label = "As vendas no Nordeste superaram as do\nSudeste nos meses de Jun e Out de 2023") +
  annotate("curve", x = 8, y = 6e5, xend = 6, yend = 3e5,
            arrow = arrow(length = unit(2, "mm"))) +
  annotate("curve", x = 8, y = 6e5, xend = 10, yend = 3e5,
...
```{r}
ggsave("imagens/g_vendas_por_regiao.png", g_vendas_por_regiao, 
       height = 5, width = 7, dpi=300)
ggsave("imagens/g_vendas_por_tri_estados_sul.png", g_vendas_por_tri_estados_sul, 
       height = 5, width = 7, dpi=300)
ggsave("imagens/g_vendas_por_mes_regiao.png", g_vendas_por_mes_regiao, 
       height = 5, width = 7, dpi=300)
```
...
-   Um Tutorial de ggplot2 para gráficos estéticamente agradáveis em R (em inglês). <https://www.cedricscherer.com/2019/08/05/a-ggplot2-tutorial-for-beautiful-plotting-in-r/#text>
